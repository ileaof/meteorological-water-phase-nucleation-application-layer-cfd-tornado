...
def rain_evaporation(st, cfg, dt):
    ...
    if not cfg.processes.rain_evaporation:
        return []
    factor = float(getattr(cfg, "rain_evaporation_factor", 1.0))
    ...
    # Preserve the exact legacy arithmetic at the default. Scale the physical
    # rate, not the capped transfer; never bypass saturation or water limits.
    if factor != 1.0:
        evap = factor * evap
