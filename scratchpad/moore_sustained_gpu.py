"""Sustained supercell in the REAL ERA5 Moore environment (GPU): mature ~28 min on a larger
domain, then a storm-following nest to develop the LOW-LEVEL mesocyclone -- and measure the
interior low-level vorticity to compare with the observed KTLX couplet (Vrot 26, vort 0.21)."""
import os, sys, time, json
REPO = r"c:/Users/ileao/OneDrive/Documentos/met_h2o_nucleation_cfd_tornado"
sys.path.insert(0, os.path.join(REPO, "src"))
import numpy as np, warnings; warnings.filterwarnings("ignore")

OUT = os.path.join(REPO, "outputs", "moore_sustained"); os.makedirs(OUT, exist_ok=True)
...
log("=== Moore sustained supercell (REAL ERA5 env, device=%s) ===" % DEV)
...
nx = 64; nz = 40; Lx = 100000.0; Lz = 15000.0
...
scfg = build_storm_config(preset="storm", nx=nx, ny=nx, nz=nz, Lx=Lx, Ly=Lx, Lz=Lz, duration=1.0,
                          dt_max=4.0, drag=True, z_stretch=1.05, C_s=0.20, device=DEV)
scfg.sim.physics.bubble_dtheta = 6.0
sim = StormSimulation(scfg, base=base)
log("parent %dx%dx%d dx=%.0f m on %s; maturing ~28 min ..." % (nx, nx, nz, sim.grid.dx, sim.grid.backend.name))
...
for step in range(1400):
    ...
    if step % 40 == 0:
        ...
        if not np.isfinite(w).all():
            log("  instability at t=%.0f -> stop maturation" % sim.t); break
        ...
        # interior only (avoid boundaries)
        b = nx // 6
        zli = float(np.abs(zl[b:-b, b:-b]).max())
        log("  t=%4.0f w_max=%5.1f low-lvl zeta(interior)=%.2e" % (sim.t, w.max(), zli))
        kl_hist.append((sim.t, float(w.max()), zli))

rr = rot.rotation_report(sim.state, sim.grid, base=sim.base)
log("PARENT matured: w_max=%.1f midmeso=%.2e" %
    (float(np.abs(np.asarray(sim.grid.backend.to_cpu(sim.state.w))).max()), rr.get("midlevel_mesocyclone", 0)))

# storm-following nest on the updraft to develop the low-level meso
to = sim.grid.backend.to_cpu; _, _, wc = rot._centered_velocity(sim.state, sim.grid); wc = np.asarray(to(wc))
...
log("storm-following nest dx=%.0f m over the updraft, window 480 s ..." % (sim.grid.dx / 3))
nest, rep = nst.run_concurrent_nest(sim, spec, window=480.0, follow=True, les_boost=1.5, cfl=0.2,
                                    progress=lambda t, w, s: log("  nest t=%4.0f/%.0f" % (t, w)) if int(t) % 120 < 5 else None)
# low-level interior vorticity of the nest
...
# rotational velocity around the low-level max (storm-relative)
...
summary = {"env": {k: float(d[k]) for k in ("CAPE_J_kg", "CIN_J_kg", "shear_0_6km_m_s") if d[k] is not None},
           "SRH03": float(snd.storm_relative_helicity(base)),
           "nest_dx_m": float(ng.dx), "low_level_zeta_interior_s": zlow_int, "low_level_Vrot_ms": vrot,
           "nest_zeta_report": float(rep["rotation"]["zeta_abs_max"]),
           "observed": {"Vrot_ms": 26.0, "vorticity_s": 0.205},
           "wall_clock_s": time.time() - _t0, "maturation_history": kl_hist}
json.dump(summary, open(os.path.join(OUT, "summary.json"), "w"), indent=2)
log("=== DONE: nest dx=%.0fm LOW-LEVEL interior zeta=%.3e Vrot=%.1f (obs 0.205/26) (%.0fs) ===" %
