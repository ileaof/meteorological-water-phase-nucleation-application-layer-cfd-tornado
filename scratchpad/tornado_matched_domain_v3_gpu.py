...
Here the FINEST domain is the SAME 4.0 km in both runs and only dx differs:
    ...
    MODE=fine   : 600 -> 200 -> 67 -> 22 m , finest = 180 cells over 4.0 km
                  (v3: the 67 m intermediate widened 6.0 -> 10.0 km, the L2 fix)
...
OUT = os.path.join(REPO, "outputs", "matched_domain_v3"); os.makedirs(OUT, exist_ok=True)
...
# L2 FIX.  The tracking diagnosis (nest_tracking_diagnosis_gpu.py) showed the cascade does NOT
# lose the vortex at the 22 m box -- it loses it at the 6 km / 67 m INTERMEDIATE, which then
# hands the finest level an already-edge-located, contaminated vortex:
#     L1 200 m /18 km  peak in sponge  0/14  mean pos 0.21  edge/interior 0.97   healthy
#     L2  67 m / 6 km  peak in sponge  7/14  mean pos 0.63  edge/interior 2.37   BREAKS
#     L3  22 m / 4 km  peak in sponge  6/14  mean pos 0.84  edge/interior 4.41   inherits it
# The standalone coarse run's 67 m level is 4 km and stays healthy (edge/interior 0.77); the
# intermediate was 6 km only because it must CONTAIN the 4 km child, leaving it almost no
# margin once its own 267 m sponge and the tracking lag are taken out.  Widen it to 10 km
# (50 parent cells) so the 4 km child sits well inside its trusted interior.
NCX = [30, 20] if MODE == "coarse" else [30, 50, 60]
# v2 FIXES (see docs): the sponge is pinned to a PHYSICAL width so it does not shrink by
# `refine` at every level (4 cells = 267 m at 67 m but only 89 m at 22 m), and the moving-nest
# tracker EXCLUDES its parent-nest's sponge band so it is not steered by edge artifacts.
SPONGE_M = float(os.environ.get("SPONGE_M", 267.0))   # == the old 4 cells at dx=67 m (no-op there)
FOLLOW_BORDER = os.environ.get("FOLLOW_BORDER", "auto")
FOLLOW_FRAC = float(os.environ.get("FOLLOW_FRAC", 0.7))
...
log("cascade %s: NCX=%s -> finest dx target %s, window %.0fs | v2: sponge=%.0fm "
    "follow_border=%s follow_filter=%.2f"
    % (MODE, NCX, "67 m" if MODE == "coarse" else "22 m", WIN, SPONGE_M, FOLLOW_BORDER,
       FOLLOW_FRAC))
sims, rep = nst.run_multilevel_nest(sim, [mkspec(i) for i in range(len(NCX))], window=WIN,
                                    ...
                                    follow_filter=FOLLOW_FRAC, follow_border=FOLLOW_BORDER,
